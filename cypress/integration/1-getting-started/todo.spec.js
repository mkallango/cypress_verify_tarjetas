...
describe('Verify and save the number of Cards', () => {  
  ...
  users.forEach((user)=>{
    it('Verify if Tarjetas link is available', () => {
      cy.visit('/')
    ...
    it('Verify how many Tarjetas the user have and save the total to a file',async ()=>{
      cy.intercept('/login').as('login')
      cy.intercept('*userCards/list').as('listCards')
      cy.visit('/userCards/list')
      cy.login(user.username, user.password)
